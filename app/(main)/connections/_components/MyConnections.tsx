      ...
      {connections.length > 0 ? (
        <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {connections.map((connection: Connection) => (
            <li
              key={connection.id}
              className="bg-green-100/50 dark:bg-slate-800 shadow-md rounded-lg p-4 flex flex-col items-center text-black dark:text-white"
            >
              <Image
                src={
                  connection.connectedUser.profilePicture || "/placeholder.svg"
                }
              ...
              <p className="text-sm text-gray-600 mb-2">
                {connection.connectedUser.given_name}{" "}
                {connection.connectedUser.family_name}
              </p>
